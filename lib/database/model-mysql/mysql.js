...
const Debounce = require('../model-abstract/debounce');
const thinkInstance = require('../model-abstract/instance');
...
class ThinkMysql {
  ...
  constructor(config) {
    ...
    this.pool.on('acquire', connection => {
      config.logConnect && debug(`acquire: Connection ${connection.threadId} acquired`);
    });
    this.pool.on('connection', () => {
      config.logConnect && debug('connection: A new connection is made within the pool');
    });
    this.pool.on('enqueue', () => {
      config.logConnect && debug('enqueue: Waiting for available connection slot');
    });
    this.pool.on('release', connection => {
      config.logConnect && debug(`release: Connection ${connection.threadId} released`);
    });
