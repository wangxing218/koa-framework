...
class ThinkMysql {
  ...
  constructor(config) {
    ...
    this.pool.on('acquire', connection => {
      config.logSql && debug(`acquire: Connection ${connection.threadId} acquired`);
    });
    this.pool.on('connection', () => {
      debug('connection: A new connection is made within the pool');
    });
    this.pool.on('enqueue', () => {
      config.logSql && debug('enqueue: Waiting for available connection slot');
    });
    this.pool.on('release', connection => {
      config.logSql && debug(`release: Connection ${connection.threadId} released`);
    });
